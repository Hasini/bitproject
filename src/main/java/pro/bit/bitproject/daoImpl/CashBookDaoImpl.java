...
public class CashBookDaoImpl implements CashBookDao{
	...
	public CashBook createCasahbook(CashBook cb) {
		String cbcreatequery = "INSERT INTO cash_book (branch_id,customer_id,bill_amount,paid_amount,paid_date,entered_user,custcashbookid,pay_type,extra_pay) VALUES (?,?,?,?,?,?,?,?,?)";
	...
	public int getmode(String custcashbookid) {
		...
		try {
			...
			PreparedStatement ps = ConnectionUtil.openConnection().prepareStatement("select mode from lend_shedule where custcashbookid = ?");
			ps.setString(1, custcashbookid);
			ResultSet rs= ps.executeQuery();
			while (rs.next()) {
				mode = rs.getInt("mode");
	...
	public boolean checkAvalablityofArr(String custcashbookid){
		...
		try {
			st = ConnectionUtil.openConnection().prepareStatement("select cust_cash_booid from tot_arrears where cust_cash_booid = ?");
	...
	public void updateArrearsAmttogetTot(String custcashbookid, LocalDateTime paiddate, double availabletotarrearsforthecus,double extrayment,double todayarr) throws Exception {
		...
		try {
			ps = ConnectionUtil.openConnection().prepareStatement("update tot_arrears set tot_arrears=?,last_updated_date=? where cust_cash_booid=?");
	...
	public double getcurrenttotarr(String custcashbookid) {
		...
		try {
			...
			PreparedStatement ps = ConnectionUtil.openConnection().prepareStatement("select cust_cash_booid,tot_arrears from tot_arrears where cust_cash_booid = ? ");
			ps.setString(1, custcashbookid);
			ResultSet rs= ps.executeQuery();
			while (rs.next()) {
